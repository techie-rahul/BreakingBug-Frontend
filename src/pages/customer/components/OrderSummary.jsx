...
const OrderSummary = ({ handleNext, handleBack }) => {
  ...
  const { currentUser, productDetailsCart } = useSelector(
    (state) => state.user
  );
          ...
          <List disablePadding>
            <ListItem sx={{ py: 1, px: 0 }}>
              <ListItemText
                ...
                secondary={`Quantity: ${productDetailsCart.quantity}`}
              />
              <Typography variant="subtitle1" sx={{ fontWeight: 700 }}>
                {`₹${
                  productDetailsCart.price &&
                  productDetailsCart.price.mrp * productDetailsCart.quantity
                }`}
              </Typography>
            </ListItem>
            <ListItem sx={{ py: 1, px: 0 }}>
              ...
              <Typography variant="subtitle1" sx={{ color: "green" }}>
                ₹
                {productDetailsCart.price &&
                  productDetailsCart.price.mrp - productDetailsCart.price.cost}
              </Typography>
            </ListItem>
            <ListItem sx={{ py: 1, px: 0 }}>
            ...
            {cartDetails.map((product, index) => (
              <ListItem key={index} sx={{ py: 1, px: 0 }}>
                ...
                <Typography variant="body2">{`₹${
                  product.quantity * product.price.mrp
                }`}</Typography>
              </ListItem>
            ))}
            <ListItem sx={{ py: 1, px: 0 }}>
